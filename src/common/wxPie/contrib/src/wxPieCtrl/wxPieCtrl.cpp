		...
		for(x = 0; x <= 2 * M_PI; x += 0.05)
			{
				...
				if(w > 0) 
				{
					...
					if(!m_ShowEdges) m_CanvasDC.SetPen(wxPen(curColour));			
					m_CanvasDC.SetBrush(wxBrush(curColour));			
				}		
				if(sin(x+m_RotationAngle)<0 && sin(x-0.05+m_RotationAngle)<=0 && x > 0)m_CanvasDC.DrawPolygon(4, points);
				if(changeangle) 			
				{			
					angleindex += 1;			
				}
			...
			if(sin(x+m_RotationAngle)<0 && sin(x-0.05+m_RotationAngle)<0)m_CanvasDC.DrawPolygon(4, points);			
//-----------------------------------------------------------------------
#ifdef __WXMSW__
			if(m_Angle <= M_PI/2)
			{		
