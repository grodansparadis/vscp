...
VSCPInformation::VSCPInformation ( void )
{
  ...
  m_hashClass[ 65 ]         = _( "VSCP_CLASS1_MEASUREZONE" );
...
void VSCPInformation::fillClassDescriptions( wxArrayString& strArray, VSCPInformationFormat format )
{
    wxString str;
    int idx;
...
void VSCPInformation::fillClassDescriptions( wxControlWithItems *pctrl, VSCPInformationFormat format )
{
    wxString str;
    int idx;
...
bool getVSCPDataCodingAsString( const vscpEvent *pEvent, wxString& strValue )
{
    ...
    if ( pEvent->sizeData < 2 ) return false;

    unsigned short type = ( 0x07 & ( pEvent->pdata[0] >> 5 ) );
    switch ( type ) {

    case 0: // series of bits
        for ( i=1; i<pEvent->sizeData; i++ ) {

            for ( j=7; j>0; j-- ) {
                
                if ( pEvent->pdata[ i ] & ( 2 ^ j ) ) {
                    strValue += wxT ( "true" );
                }
                else {
                    strValue += wxT ( "false" );
    ...
    case 1: // series of bytes
        for ( i=1; i<pEvent->sizeData; i++ ) {
            strValue += wxString::Format( wxT( "%d" ), pEvent->pdata[ i ] );
                ...
                case 3:
                    {
                        ...
                        memset( construct, 0, sizeof( construct ) );
                        construct[ 2 ] = pEvent->pdata[ 1 ];
                        construct[ 3 ] = pEvent->pdata[ 2 ];
                        lo = *((long *)construct);
                        lo = wxUINT32_SWAP_ON_LE( lo );
                    }
                    break;
                ...
                case 4:
                    {
                        ...
                        memset( construct, 0, sizeof( construct ) );
                        construct[ 1 ] = pEvent->pdata[ 1 ];
                        construct[ 2 ] = pEvent->pdata[ 2 ];
                        construct[ 3 ] = pEvent->pdata[ 3 ];
                        lo = *((long *)construct);
                        lo = wxUINT32_SWAP_ON_LE( lo );
                    }
                    break;
                ...
                case 5:
                    {
                        ...
                        memset( construct, 0, sizeof( construct ) );
                        construct[ 0 ] = pEvent->pdata[ 1 ];
                        construct[ 1 ] = pEvent->pdata[ 2 ];
                        construct[ 2 ] = pEvent->pdata[ 3 ];
                        construct[ 3 ] = pEvent->pdata[ 4 ];
                        lo = *((long *)construct);
                        lo = wxUINT32_SWAP_ON_LE( lo );
                    }
                    break;
    ...
    case 5: // Floating point value
        {
            ...
            // Get the exponent
            exponent = ( pEvent->pdata[ 1 ] << 1 );
            if ( pEvent->pdata[ 2 ] & 0x80 ) {
                exponent = exponent | 1;
            }
            ...
            wxUint32 value = *((uint32_t *)(pEvent->pdata + 1));
  ...
  case VSCP_CLASS1_MEASUREMENT:
    {
      ...
      str = wxString::Format( _("Sensor %d "), VSCP_DATACODING_INDEX( *pEvent->pdata ) ); 
      ...
      switch ( VSCP_DATACODING_TYPE( *pEvent->pdata ) & 0xE0 ) {

      case 0x00:  // bit format
        {
			str += _("[bit] = ");
			//char sth[8];
			str += wxString::Format(wxT("%X"), (long)getDataCodingBitArray( pEvent->pdata, pEvent->sizeData ));	  
        }
        break;
      ...
      case 0x40:  // string format
        {
          str += _("[string] = ");
          str += getDataCodingString( pEvent->pdata, pEvent->sizeData );
        }
        break;
      ...
      case 0x80:  // normalized int format
        {
          double temp  = getDataCodingNormalizedInteger( pEvent->pdata, pEvent->sizeData );
          str += wxString::Format( _("[nint] = %f "), temp );
        }
        break;
      case 0xA0:  // float format
        float msrmt = getDataCodingFloat(pEvent->pdata, pEvent->sizeData );
      ...
      case VSCP_TYPE_MEASUREMENT_LENGTH:
        {
          switch ( VSCP_DATACODING_UNIT( *pEvent->pdata ) ) {
  ...
  case VSCP_CLASS1_INFORMATION:
    ...
    case VSCP_TYPE_INFORMATION_BUTTON:
      if ( pEvent->sizeData >= 3 ) {
        ...
        // Key type code
        if ( 0 == ( pEvent->pdata[ 0 ] & 0x03 ) ) {
          str = _("Button released.");
        }
        else if( 1 == ( pEvent->pdata[ 0 ] & 0x03 ) ) {
          str = _("Button pressed.");
        }
        else if( 2 == ( pEvent->pdata[ 0 ] & 0x03 ) ) {
          str = _("Keycode.");
        }                
        else {
        ...
        str += wxString::Format( _("Button Code=%d\n"), 
          ( ( pEvent->pdata[ 3 ] << 8 ) + pEvent->pdata[ 4 ] ) ); 

        str += wxString::Format( _("Code Page=%d\n"), 
          ( ( pEvent->pdata[ 5 ] << 8 ) + pEvent->pdata[ 6 ] ) );                            
      }
      break;
    ...
    case VSCP_TYPE_INFORMATION_TOKEN_ACTIVITY:
      {
        if ( pEvent->sizeData >= 5 ) {
          ...
          str = wxString::Format( _("Token activity.\n Event Code=%d ["), ( *pEvent->pdata & 0x03) ); 

          // Event Code
          switch ( *pEvent->pdata & 0x03) {
