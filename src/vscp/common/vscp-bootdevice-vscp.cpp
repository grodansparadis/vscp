...
int
CBootDevice_VSCP::writeBlock(uint32_t block, const uint8_t *paddr)
{
  ...
  if ((crc_packet != crc_block) || (block_packet != block)) {
    ...
    */
    ...
    while (block < nBlocks) {
      ...
      while (trycnt > 0) {
        ...
        // Write the block
        if (VSCP_ERROR_SUCCESS != (rv = writeBlock(block, pbuf + (block * m_blockSize)))) {
