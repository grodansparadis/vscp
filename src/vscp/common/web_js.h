...
#define WEB_COMMON_JS "<script type=\"text/javascript\">"\
"sfHover=function(){ "\
"var b=document.getElementById(\"nav\").getElementsByTagName(\"LI\");"\
"for(var a=0;a<b.length;a++){b[a].onmouseover=function(){this.className+=\" "\
"sfhover\"};b[a].onmouseout=function(){this.className=this.className."\
"replace(new RegExp(\" sfhover\\\\b\"),\"\")}}};if(window.attachEvent)"\
"{window.attachEvent(\"onload\",sfHover)}function ChangeColor(a,b){if(b)"\
"{a.style.backgroundColor=\"#dcfac9\"}else{a.style.backgroundColor=\"white\"}}"\
"function DoNav(a){document.location.href=a};"\
"</script>"
