...
////@begin frmDeviceConfig content construction
  ...
  itemMenu9->Append(ID_MENUITEM_EDIT_CUT, _("Cut"), wxEmptyString, wxITEM_NORMAL);
  itemMenu9->Append(ID_MENUITEM_EDIT_COPY, _("Copy"), wxEmptyString, wxITEM_NORMAL);
  itemMenu9->Append(ID_MENUITEM_EDIT_PASTE, _("Paste"), wxEmptyString, wxITEM_NORMAL);
  menuBar->Append(itemMenu9, _("Edit"));
  wxMenu* itemMenu13 = new wxMenu;
  itemMenu13->Append(ID_MENUITEM_SCAN, _("Scan for nodes..."), wxEmptyString, wxITEM_NORMAL);
  itemMenu13->Append(ID_MENUITEM_SET_NICKNAME, _("Set node nickname..."), wxEmptyString, wxITEM_NORMAL);
  itemMenu13->Append(ID_MENUITEM_DROP_NICKNAME, _("Reset node (drop nickname)..."), wxEmptyString, wxITEM_NORMAL);
  itemMenu13->Append(ID_MENUITEM_GUID_RESET_NODE, _("GUID reset node (drop nickname)..."), wxEmptyString, wxITEM_NORMAL);
  itemMenu13->Append(ID_MENUITEM_INCREMENT_REGISTER, _("Increment register..."), wxEmptyString, wxITEM_NORMAL);
  itemMenu13->Append(ID_MENUITEM_DECREMENT_REGISTER, _("Decrement register..."), wxEmptyString, wxITEM_NORMAL);
  itemMenu13->AppendSeparator();
  itemMenu13->Append(ID_MENUITEM_READ_REGISTER, _("Read register..."), wxEmptyString, wxITEM_NORMAL);
  itemMenu13->Append(ID_MENUITEM_READ_PAGE, _("Read page..."), wxEmptyString, wxITEM_NORMAL);
  itemMenu13->Append(ID_MENUITEM_WRITE_REGISTER, _("Write register..."), wxEmptyString, wxITEM_NORMAL);
  itemMenu13->Append(ID_MENUITEM_CLONE_NODE, _("Clone node..."), wxEmptyString, wxITEM_NORMAL);
  menuBar->Append(itemMenu13, _("Tools"));
  wxMenu* itemMenu25 = new wxMenu;
  itemMenu25->Append(ID_MENUITEM_HELP, _("VSCP-Works Help"), wxEmptyString, wxITEM_NORMAL);
  itemMenu25->Append(ID_MENUITEM_FAQ, _("Frequently Asked Questions"), wxEmptyString, wxITEM_NORMAL);
  itemMenu25->Append(ID_MENUITEM_KB_SHRTCUTS, _("Keyboard shortcuts"), wxEmptyString, wxITEM_NORMAL);
  itemMenu25->AppendSeparator();
  itemMenu25->Append(ID_MENUITEM_THANKS, _("Thanks..."), wxEmptyString, wxITEM_NORMAL);
  itemMenu25->Append(ID_MENUITEM_CREDITS, _("Credits..."), wxEmptyString, wxITEM_NORMAL);
  itemMenu25->AppendSeparator();
  itemMenu25->Append(ID_MENUITEM_GO_VSCP_SITE, _("Go to VSCP site"), wxEmptyString, wxITEM_NORMAL);
  itemMenu25->AppendSeparator();
  itemMenu25->Append(ID_MENUITEM_ABOUT, _("About"), wxEmptyString, wxITEM_NORMAL);
  menuBar->Append(itemMenu25, _("Help"));
  wxMenu* itemMenu36 = new wxMenu;
  menuBar->Append(itemMenu36, _("Menu"));
  wxMenu* itemMenu37 = new wxMenu;
  menuBar->Append(itemMenu37, _("Menu"));
  itemFrame1->SetMenuBar(menuBar);

  wxToolBar* itemToolBar38 = CreateToolBar( wxTB_FLAT|wxTB_HORIZONTAL, ID_TOOLBAR_DEVICE_CONFIG );
  wxBitmap itemtool39Bitmap(itemFrame1->GetBitmapResource(wxT("open.xpm")));
  wxBitmap itemtool39BitmapDisabled;
  itemToolBar38->AddTool(ID_TOOL6, wxEmptyString, itemtool39Bitmap, itemtool39BitmapDisabled, wxITEM_NORMAL, _("Fetch data from file"), wxEmptyString);
  wxBitmap itemtool40Bitmap(itemFrame1->GetBitmapResource(wxT("save.xpm")));
  wxBitmap itemtool40BitmapDisabled;
  itemToolBar38->AddTool(ID_TOOL7, wxEmptyString, itemtool40Bitmap, itemtool40BitmapDisabled, wxITEM_NORMAL, _("Save data to file"), wxEmptyString);
  itemToolBar38->AddSeparator();
  wxBitmap itemtool42Bitmap(itemFrame1->GetBitmapResource(wxT("cut.xpm")));
  wxBitmap itemtool42BitmapDisabled;
  itemToolBar38->AddTool(ID_TOOL8, wxEmptyString, itemtool42Bitmap, itemtool42BitmapDisabled, wxITEM_NORMAL, _("Remove selected row(s)"), wxEmptyString);
  wxBitmap itemtool43Bitmap(itemFrame1->GetBitmapResource(wxT("copy.xpm")));
  wxBitmap itemtool43BitmapDisabled;
  itemToolBar38->AddTool(ID_TOOL9, wxEmptyString, itemtool43Bitmap, itemtool43BitmapDisabled, wxITEM_NORMAL, _("Copy selected row(s) \nto the clipboard"), wxEmptyString);
  wxBitmap itemtool44Bitmap(itemFrame1->GetBitmapResource(wxT("paste.xpm")));
  wxBitmap itemtool44BitmapDisabled;
  itemToolBar38->AddTool(ID_TOOL10, wxEmptyString, itemtool44Bitmap, itemtool44BitmapDisabled, wxITEM_NORMAL, _("Paste row(s) from clipboard"), wxEmptyString);
  itemToolBar38->AddSeparator();
  wxBitmap itemtool46Bitmap(itemFrame1->GetBitmapResource(wxT("Print.xpm")));
  wxBitmap itemtool46BitmapDisabled;
  itemToolBar38->AddTool(ID_TOOL11, wxEmptyString, itemtool46Bitmap, itemtool46BitmapDisabled, wxITEM_NORMAL, _("Print selected or all row(s)"), wxEmptyString);
  itemToolBar38->AddSeparator();
  wxStaticText* itemStaticText48 = new wxStaticText;
  itemStaticText48->Create( itemToolBar38, wxID_STATIC, _("Node id: "), wxDefaultPosition, wxDefaultSize, 0 );
  itemStaticText48->SetFont(wxFont(8, wxSWISS, wxNORMAL, wxBOLD, false, wxT("Tahoma")));
  itemToolBar38->AddControl(itemStaticText48);
  wxArrayString m_comboNodeIDStrings;
  m_comboNodeID = new wxComboBox;
  m_comboNodeID->Create( itemToolBar38, ID_COMBOBOX4, wxEmptyString, wxDefaultPosition, wxSize(370, -1), m_comboNodeIDStrings, wxCB_DROPDOWN );
  ...
  itemToolBar38->AddControl(m_BtnActivateInterface);
...
bool frmDeviceConfig::getLevel1DmInfo( unsigned char nodeid, unsigned char *pdata )
{
    bool rv = true;
    int errors = 0;
    bool bResend;
    ...
    canalEvent.obid = 0;
    canalEvent.id = ( VSCP_TYPE_PROTOCOL_GET_MATRIX_INFO << 8 );
    canalEvent.sizeData = 1;
    canalEvent.data[ 0 ] = nodeid;    // Node to read from

    bResend = false;
    m_csw.doCmdSend( &canalEvent );
 
    wxLongLong startTime = ::wxGetLocalTimeMillis();

    while ( true ) {

        if ( m_csw.doCmdDataAvailable() ) {
            if ( CANAL_ERROR_SUCCESS == m_csw.doCmdReceive( &canalEvent ) ) {
                if ( (unsigned short)( canalEvent.id & 0xffff ) ==
                        ( ( VSCP_TYPE_PROTOCOL_GET_MATRIX_INFO_RESPONSE << 8 ) + 
                            nodeid ) ) {    // DM info reply?
...
bool frmDeviceConfig::getLevel2DmInfo( unsigned char *interfaceGUID, unsigned char *pdata )
{
    ...
    while ( true ) {
        ...
        if ( m_csw.doCmdDataAvailable() ) {                                 // Message available
            if ( CANAL_ERROR_SUCCESS == m_csw.doCmdReceive( &event ) ) {    // Valid event
                if ( ( VSCP_CLASS1_PROTOCOL == event.vscp_class ) &&                           
                        ( VSCP_TYPE_PROTOCOL_GET_EMBEDDED_MDF_RESPONSE == event.vscp_type ) ) { // DM reply?   
...
void frmDeviceConfig::OnButtonUpdateClick( wxCommandEvent& event )
{
    ...
    if ( m_chkFullUppdate->GetValue() || m_bFirstRead ) {
        ...
        progressDlg.Pulse( _("Fetching device status") );

        // Load MDF
        if ( rv ) OnButtonLoadMdfClick( event );

        // Write status information
        if ( rv ) writeStatusInfo();

    }

    // Don't save registers on next run
    if ( rv ) m_bFirstRead = false;
  
    ::wxEndBusyCursor();
  
    // Enable undo and load buttons
    if ( rv ) m_ctrlButtonLoadMDF->Enable( true );
    if ( rv ) m_ctrlButtonUndo->Enable( true );
  
    // Set position
    if ( m_gridRegisters->IsSelection() ) {
        m_gridRegisters->MakeCellVisible( m_lastLeftClickCol, 
                                            m_lastLeftClickRow );
    }
  
    // Repaint the window
    m_gridRegisters->ForceRefresh();
...
void frmDeviceConfig::OnButtonLoadDefaultsClick( wxCommandEvent& event )
{
    ...
    if ( m_chkMdfFromFile->GetValue() ) {
        ...
        if ( !m_mdf.downLoadMDF( remoteFile, localFile ) ) {
    ...
    for ( iter = m_mdf.m_list_register.begin(); iter != m_mdf.m_list_register.end(); ++iter ) {
            ...
            switch ( reg->m_nPage ) {
                case 0:	
                    ...
                    str.Printf( reg->m_strName + 
                                    _("\n") + 
                                    reg->m_strDescription );
                    m_gridRegisters->SetCellValue( reg->m_nOffset, 
		            3,  
					str );
                    ...
                    // Make all parts of the row visible
                    m_gridRegisters->AutoSizeRow( reg->m_nOffset );
                    break;
                }
            }
        }
        ...
        if ( rv ) m_ctrlButtonWizard->Enable( true );
...
void frmDeviceConfig::OnLeftDClick( wxGridEvent& event )
{
    ...
    if ( ID_GRID_REGISTERS == event.GetId() ) {
        ...
        if ( wxID_OK == dlg.ShowModal() ) {
            ...
            dlg.TransferDataFromWindow( strValue );
            
            m_gridAbstractions->SetCellValue( event.GetRow(), 3, strValue );
			...
			switch ( m_mdf.m_list_abstraction[ event.GetRow() ]->m_nType ) {
				...
				case type_string: 
					...
					for ( pos=0; pos<strValue.Length(); pos++ ) {
						...
						strBuf.Printf( _("0x%02lx"), strValue[ pos ] );
						m_gridRegisters->SetCellValue( 
											m_mdf.m_list_abstraction[ event.GetRow() ]->m_nOffset + pos, 
                                            2, 
                                            strBuf );
				...
				case type_bitfield:
					...
					for ( pos = strValue.Length(); pos > 0; pos-- ) {
						if 	( '1' == strValue[ pos - 1 ] ) {
							val |= ( 1 << ( strValue.Length() - pos ) ); 	
						}
						// Just handle eight bits
						if ( pos < ( strValue.Length() - 8 ) ) break;
					}
				...
				case type_int8_t:
					strBuf.Printf( _("0x%02lx"), readStringValue( strValue ) );
					m_gridRegisters->SetCellValue( 
											m_mdf.m_list_abstraction[ event.GetRow() ]->m_nOffset, 
                                            2, 
                                            strBuf );
				...
				case type_uint32_t:
					{
						...
						strValue.ToULong( &ulongVal );

						uint8_t *pVal = (uint8_t *)&ulongVal;
						for ( int i=0; i<4; i++ ) {
				...
				case type_uint64_t:
					{
						wxULongLong_t  ulonglongVal;
						strValue.ToULongLong( &ulonglongVal );

						uint8_t *pVal = (uint8_t *)&ulonglongVal;
						for ( int i=0; i<8; i++ ) {
				...
				case type_date:
					{
						...
						strBuf.Printf( _("0x%02lx"), date.GetDay() );
						m_gridRegisters->SetCellValue( 
											m_mdf.m_list_abstraction[ event.GetRow() ]->m_nOffset, 
                                            2, 
                                            strBuf );

						strBuf.Printf( _("0x%02lx"), date.GetMonth() );
						m_gridRegisters->SetCellValue( 
											m_mdf.m_list_abstraction[ event.GetRow() ]->m_nOffset + 1, 
                                            2, 
                                            strBuf );

						strBuf.Printf( _("0x%02lx"), date.GetYear() >> 8 );
						m_gridRegisters->SetCellValue( 
											m_mdf.m_list_abstraction[ event.GetRow() ]->m_nOffset + 2, 
                                            2, 
                                            strBuf );

						strBuf.Printf( _("0x%02lx"), date.GetYear() & 0xff );
						m_gridRegisters->SetCellValue( 
											m_mdf.m_list_abstraction[ event.GetRow() ]->m_nOffset + 3, 
                                            2, 
                                            strBuf );
				...
				case type_time:
					{
						...
						strBuf.Printf( _("0x%02lx"), time.GetHour() );
						m_gridRegisters->SetCellValue( 
											m_mdf.m_list_abstraction[ event.GetRow() ]->m_nOffset, 
                                            2, 
                                            strBuf );

						strBuf.Printf( _("0x%02lx"), time.GetMinute() );
						m_gridRegisters->SetCellValue( 
											m_mdf.m_list_abstraction[ event.GetRow() ]->m_nOffset + 1, 
                                            2, 
                                            strBuf );

						strBuf.Printf( _("0x%02lx"), time.GetSecond() );
						m_gridRegisters->SetCellValue( 
											m_mdf.m_list_abstraction[ event.GetRow() ]->m_nOffset + 2, 
                                            2, 
                                            strBuf );
        ...
        if ( wxID_OK == dlg.ShowModal() ) {
            ...
            for ( int i=0; i<m_mdf.m_dmInfo.m_nRowSize; i++ ) {
                m_gridRegisters->SetCellTextColour( 
                    i + m_mdf.m_dmInfo.m_nStartOffset + 
                        m_mdf.m_dmInfo.m_nRowSize * event.GetRow(), 
                    2, 
                    *wxRED );   
            }
