...
main(void)
{
    ...
    for (i = 0; i < NUMBER_OF_THREADS; i++) {
        ...
        if (pthread_create(&workthreads[i], &thread_attr, workThread, &i)) {
        }
    }
...
workThread(void *id)
{
    ...
    // * * * * CANAL OPEN * * * *
    if (0 ==
        (m_proc_CanalOpen = (LPFNDLL_CANALOPEN)dlsym(handle, "CanalOpen"))) {

        // Free the library
        dlclose(handle);
        fprintf(stderr,
                "test: Unable to get dl entry for CanalOpen. error=%s",
                dlerror());
        exit(EXIT_FAILURE);
    }
    ...
    // * * * * CANAL CLOSE * * * *
    if (0 ==
        (m_proc_CanalClose = (LPFNDLL_CANALCLOSE)dlsym(handle, "CanalClose"))) {

        // Free the library
        dlclose(handle);
        fprintf(stderr,
                "test: Unable to get dl entry for CanalClose. error=%s",
                dlerror());
        exit(EXIT_FAILURE);
    }

    // * * * * CANAL GETLEVEL * * * *
    if (0 == (m_proc_CanalGetLevel =
                (LPFNDLL_CANALGETLEVEL)dlsym(handle, "CanalGetLevel"))) {

        // Free the library
        dlclose(handle);
        fprintf(stderr,
                "test: Unable to get dl entry for CanalGetLevel. error=%s",
                dlerror());
        exit(EXIT_FAILURE);
    }
    ...
    // * * * * CANAL SEND * * * *
    if (0 ==
        (m_proc_CanalSend = (LPFNDLL_CANALSEND)dlsym(handle, "CanalSend"))) {

        // Free the library
        dlclose(handle);
        fprintf(stderr,
                "test: Unable to get dl entry for CanalSend. error=%s",
                dlerror());
        exit(EXIT_FAILURE);
    }

    // * * * * CANAL DATA AVAILABLE * * * *
    if (0 == (m_proc_CanalDataAvailable = (LPFNDLL_CANALDATAAVAILABLE)dlsym(
                handle, "CanalDataAvailable"))) {

        // Free the library
        dlclose(handle);
        fprintf(stderr,
                "test: Unable to get dl entry for CanalDataAvailable. error=%s",
                dlerror());
        exit(EXIT_FAILURE);
    }
    ...
    // * * * * CANAL RECEIVE * * * *
    if (0 == (m_proc_CanalReceive =
                (LPFNDLL_CANALRECEIVE)dlsym(handle, "CanalReceive"))) {

        // Free the library
        dlclose(handle);
        fprintf(stderr,
                "test: Unable to get dl entry for CanalReceive. error=%s",
                dlerror());
        exit(EXIT_FAILURE);
    }

    // * * * * CANAL GET STATUS * * * *
    if (0 == (m_proc_CanalGetStatus =
                (LPFNDLL_CANALGETSTATUS)dlsym(handle, "CanalGetStatus"))) {

        // Free the library
        dlclose(handle);
        fprintf(stderr,
                "test: Unable to get dl entry for CanalGetStatus. error=%s",
                dlerror());
        exit(EXIT_FAILURE);
    }

    // * * * * CANAL GET STATISTICS * * * *
    if (0 == (m_proc_CanalGetStatistics = (LPFNDLL_CANALGETSTATISTICS)dlsym(
                handle, "CanalGetStatistics"))) {

        // Free the library
        dlclose(handle);
        fprintf(stderr,
                "test: Unable to get dl entry for CanalGetStatistics. error=%s",
                dlerror());
        exit(EXIT_FAILURE);
    }

    // * * * * CANAL SET FILTER * * * *
    if (0 == (m_proc_CanalSetFilter =
                (LPFNDLL_CANALSETFILTER)dlsym(handle, "CanalSetFilter"))) {

        // Free the library
        dlclose(handle);
        fprintf(stderr,
                "test: Unable to get dl entry for CanalSetFilter. error=%s",
                dlerror());
        exit(EXIT_FAILURE);
    }

    // * * * * CANAL SET MASK * * * *
    if (0 == (m_proc_CanalSetMask =
                (LPFNDLL_CANALSETMASK)dlsym(handle, "CanalSetMask"))) {

        // Free the library
        dlclose(handle);
        fprintf(stderr,
                "test: Unable to get dl entry for CanalSetMask. error=%s",
                dlerror());
        exit(EXIT_FAILURE);
    }

    // * * * * CANAL GET VERSION * * * *
    if (0 == (m_proc_CanalGetVersion =
                (LPFNDLL_CANALGETVERSION)dlsym(handle, "CanalGetVersion"))) {

        // Free the library
        dlclose(handle);
        fprintf(stderr,
                "test: Unable to get dl entry for CanalGetVersion. error=%s",
                dlerror());
        exit(EXIT_FAILURE);
    }

    // * * * * CANAL GET DLL VERSION * * * *
    if (0 == (m_proc_CanalGetDllVersion = (LPFNDLL_CANALGETDLLVERSION)dlsym(
                handle, "CanalGetDllVersion"))) {

        // Free the library
        dlclose(handle);
        fprintf(stderr,
                "test: Unable to get dl entry for CanalGetDllVersion. error=%s",
                dlerror());
        exit(EXIT_FAILURE);
    }

    // * * * * CANAL GET VENDOR STRING * * * *
    if (0 == (m_proc_CanalGetVendorString = (LPFNDLL_CANALGETVENDORSTRING)dlsym(
                handle, "CanalGetVendorString"))) {

        // Free the library
        dlclose(handle);
        fprintf(
          stderr,
          "test: Unable to get dl entry for CanalGetVendorString. error=%s",
          dlerror());
        exit(EXIT_FAILURE);
    }
