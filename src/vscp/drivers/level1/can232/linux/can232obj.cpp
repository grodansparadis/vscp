...
// $Date: 2011/07/28 20:35:07 $                                  
// $Author: akhe, ice@geomi.org $                                              
// $Revision: 0.1 $ 
///////////////////////////////////////////////////////////////////////////////
...
bool CCAN232Obj::open( const char *pDevice, unsigned long flags )
{
	...
	// Baudrate
	p = strtok( NULL, ";" );
	if ( NULL != p ) { strncpy( nBaudRate, p, strlen(p) ); }
	...
	while(!CANClose) {
		printf("C Write ["); for(int i=0; i<strlen(szCmd); i++) { printf(" %02X", szCmd[i]); } printf("]\n");
		...
		printf("C Read  ["); for(int i=0; i<cnt; i++) { printf(" %02X", szResponse[i]); } printf("]\n");
		SLEEP(100);
		if( try_count++ > 10 ) {
	...
	while(!GETVersion) {
		printf("V Write ["); for(int i=0; i<strlen(szCmd); i++) { printf("%02X ", szCmd[i]); } printf("]\n");
		m_can232obj.m_comm.comm_puts( szCmd, strlen( szCmd ), true );
		//*szResponse = 0;
		//for (int i=0; i<sizeof(szResponse); i++) szResponse[i]=0;
		cnt = m_can232obj.m_comm.comm_gets( szResponse, sizeof(szResponse), 10000 );
		printf("V Read  ["); for(int i=0; i<cnt; i++) { printf("%02X ", szResponse[i]); } printf("]\n");
	...
	*szResponse = 0;
	cnt = m_can232obj.m_comm.comm_gets( szResponse, sizeof(szResponse), 10000 );
	printf("sS ["); for(int i=0; i<cnt; i++) { printf("%02X ", szResponse[i]); } printf("]\n");

	//----------------------------------------------------------------------
	// Open CAN Bus
	//----------------------------------------------------------------------
	sprintf( szCmd, "O\r" );
	printf("O Write ["); for(int i=0; i<strlen(szCmd); i++) { printf("%02X ", szCmd[i]); } printf("]\n");
	if ( !m_can232obj.m_comm.comm_puts( szCmd, strlen( szCmd ), true ) ) {
		m_can232obj.m_bRun = false;
		m_can232obj.m_comm.close();
		return 0;
	...
	*szResponse = 0;
	cnt = m_can232obj.m_comm.comm_gets( szResponse, sizeof(szResponse), 10000 );
	printf("O Read  ["); for(int i=0; i<cnt; i++) { printf("%02X ", szResponse[i]); } printf("]\n");
	...
	//----------------------------------------------------------------------
		...
		sprintf( szCmd, "P\r" );
		printf("P Write ["); for(int i=0; i<strlen(szCmd); i++) { printf("%02X ", szCmd[i]); } printf("]\n");
                m_can232obj.m_comm.comm_puts( szCmd, strlen( szCmd ), true );
		*szResponse = 0;
		cnt = m_can232obj.m_comm.comm_gets( szResponse, sizeof(szResponse), 10000 );
		printf("P Read  ["); for(int i=0; i<cnt; i++) { printf("%02X ", szResponse[i]); } printf("]\n");
                if ( 0x07 == *(szResponse) ) {
		...
		//printf("workThread - Receive\n");
                ...
                while ( -1 != cnt ) {
                        ...
                        if ( CAN232_STATE_NONE == pcan232obj->m_can232obj.m_state ) { 
								...
								printf("workThread R - m_receiveBuf ["); for(int i=0; i<cnt; i++) {printf("%02X ",pcan232obj->m_can232obj.m_receiveBuf[i]);} printf("]\n");
...
//                                ( NULL != pcan232obj->m_can232obj.m_sndList.pHead->pObject ) ) {
		...
		if ( ( NULL != pcan232obj->m_can232obj.m_sndList.pHead ) &&
			...
			printf("workThread T - Write ["); for(int i=0; i<strlen(buf); i++) { if( buf[i] == 0x0D ){ printf("[CR]"); } else { printf("%c", buf[i]); } } printf("]\n");
			pcan232obj->m_can232obj.m_comm.comm_gets( szResponse, sizeof( szResponse ), 10000 );
			printf("workThread T - Read  ["); for(int i=0; i<strlen(szResponse); i++) {if(szResponse[i] == 0x0D){ printf("[CR]");} else { printf("%c", szResponse[i]); } } printf("]\n\n");
