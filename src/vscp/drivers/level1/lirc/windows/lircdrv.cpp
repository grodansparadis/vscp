//
// lircdrv.cpp : Defines the entry point for the DLL application.
//
...
//
// $RCSfile: lircdrv.cpp,v $                                       
// $Date: 2005/08/01 10:20:14 $                                  
// $Author: akhe $                                              
// $Revision: 1.3 $ 
...
#endif
{
	CLircObj *pdrvObj =  theApp->getDriverObject( handle );
...
#endif
{
	CLircObj *pdrvObj =  theApp->getDriverObject( handle );
	if ( NULL == pdrvObj ) return 0;
	return ( pdrvObj->dataAvailable() ? CANAL_ERROR_SUCCESS : CANAL_ERROR_GENERIC );
}
