...
CDllDrvObj::CDllDrvObj()
{
	m_instanceCounter = 0;
#ifdef WIN32
	m_objMutex = CreateMutex( NULL, true, TEXT("__CANAL_CANALLOGGER_MUTEX__") );
#else
...
CDllDrvObj::~CDllDrvObj()
{
	...
	for ( int i = 0; i<CANAL_LOGGER_DRIVER_MAX_OPEN; i++ ) {
		...
		if ( NULL != m_drvObjArray[ i ] ) {
			...
			CLog *pdrvObj =  getDriverObject( i );
			if ( NULL != pdrvObj ) { 
				pdrvObj->close();	
...
long CDllDrvObj::addDriverObject( CLog *pdrvObj )
{
	long h = 0;
...
CLog * CDllDrvObj::getDriverObject( long h )
{
	long idx = h - 1681;
