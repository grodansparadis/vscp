// TcpDrvObj.cpp: implementation of the TcpDrvObj class.
//
//////////////////////////////////////////////////////////////////////
...
bool TcpDrvObj::init( char * szHost, int port )
{
	bool rv = true;
	DWORD dwThreadId;	

	m_udpReceiveMutex = CreateMutex( NULL, true, TCP_SOCKET_DLL_RECEIVE_OBJ_MUTEX );

	// Create the TCP Transmit thread.
...
bool TcpDrvObj::send( PCANALMSG pMsg )
{
	bool rv = false;
	...
	if ( NULL != pMsg ) {
		...
		if ( m_sndList.nCount < TCP_SOCKET_MAX_SNDMSG ) {
...
bool TcpDrvObj::receive( PCANALMSG pMsg )
{
	bool rv = false;
...
int TcpDrvObj::dataAvailable( void )
{
	int cnt;
...
bool TcpDrvObj::getStatistics( PCANALSTATISTICS pCanalStatistics )
{
...
bool TcpDrvObj::getStatus( PCANALSTATUS pCanalStatus )
{
...
void transmitThread( void *pObject )
{
	...
	TcpDrvObj *pobj = (TcpDrvObj *)pObject;
	...
	// Open the mutex
	UDPObjMutex = OpenMutex( MUTEX_ALL_ACCESS, false, TCP_SOCKET_DLL_RECEIVE_OBJ_MUTEX );
...
void receiveThread( void *pObject )
{
	...
	udpReceiveMutex = OpenMutex( MUTEX_ALL_ACCESS, false, TCP_SOCKET_DLL_RECEIVE_OBJ_MUTEX );
	...
	saServer.sin_port = htons( CANAL_TCP_PORT_ID );	   
	...
	while ( pobj->m_bRun ) {
		...
		if ( NULL != pMsg ) {
			...
			*(p+0) = buf[ CANAL_SEQPOS_TIMESTAMP  + 3 ];
			...
			if (  pobj->m_rcvList.nCount < TCP_SOCKET_MAX_RCVMSG ) {
