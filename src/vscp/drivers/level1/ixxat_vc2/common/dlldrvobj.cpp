...
CDllDrvObj::CDllDrvObj()
{
	...
	for ( int i = 0; i<CANAL_IXXATVCI_DRIVER_MAX_OPEN; i++ ) {
...
CDllDrvObj::~CDllDrvObj()
{
	...
	for ( int i = 0; i<CANAL_IXXATVCI_DRIVER_MAX_OPEN; i++ ) {
		...
		if ( NULL == m_drvObjArray[ i ] ) {
			...
			CIxxObj *pdrvObj =  getDriverObject( i );
			if ( NULL != pdrvObj ) { 
				pdrvObj->close();	
...
long CDllDrvObj::addDriverObject( CIxxObj *pdrvObj )
{
	long h = 0;
...
CIxxObj * CDllDrvObj::getDriverObject( long h )
{
	long idx = h - 1681;

	// Check if valid handle
	if ( idx < 0 ) return NULL;
	if ( idx >= CANAL_IXXATVCI_DRIVER_MAX_OPEN ) return NULL;
...
void CDllDrvObj::removeDriverObject( long h )
{
	...
	// Check if valid handle
	if ( idx < 0 ) return;
	if ( idx >= CANAL_IXXATVCI_DRIVER_MAX_OPEN ) return;
