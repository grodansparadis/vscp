//
// usb2candrv.cpp : Defines the entry point for the DLL application.
//
...
// Copyright (C) 2004,2005, VSCP Team. info@vscp.org
...
//#define CANAL_SUB_VERSION	
    ...
    strcat( r_str, ";");
	//sprintf_s(tmp_str,sizeof(tmp_str),"%08X",CANAL_VERSION);
    sprintf_s(tmp_str,sizeof(tmp_str),"%d.%d.%d",CANAL_MAIN_VERSION,CANAL_MINOR_VERSION,CANAL_SUB_VERSION);
	strcat( r_str,tmp_str );
	strcat( r_str, ";");
	//sprintf_s(tmp_str,sizeof(tmp_str),"%08X",DLL_VERSION);
    sprintf_s(tmp_str,sizeof(tmp_str),"%d.%d.%d",(UCHAR)(DLL_VERSION>>16),(UCHAR)(DLL_VERSION>>8),(UCHAR)DLL_VERSION);
	strcat( r_str,tmp_str );
	strcat( r_str, ";");
	strcat( r_str, "www.edevices.lt\0");
