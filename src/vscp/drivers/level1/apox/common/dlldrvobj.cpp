...
long CDllDrvObj::addDriverObject( CApoxObj *pdrvObj )
{
	long h = 0;
...
CApoxObj * CDllDrvObj::getDriverObject( long h )
{
	long idx = h - 1681;

	// Check if valid handle
	if ( idx < 0 ) return NULL;
	if ( idx >= CANAL_APOX_DRIVER_MAX_OPEN ) return NULL;
...
void CDllDrvObj::removeDriverObject( long h )
{
	...
	// Check if valid handle
	if ( idx < 0 ) return;
	if ( idx >= CANAL_APOX_DRIVER_MAX_OPEN ) return;
