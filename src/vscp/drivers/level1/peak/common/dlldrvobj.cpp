...
CDllDrvObj::CDllDrvObj()
{
	m_instanceCounter = 0;
#ifdef WIN32
	m_objMutex = CreateMutex( NULL, true, TEXT("__CANAL_PEAK_MUTEX__") );
#else
...
long CDllDrvObj::addDriverObject( CPeakObj *pdrvObj )
{
	long h = 0;
...
CPeakObj * CDllDrvObj::getDriverObject( long h )
{
	long idx = h - 1681;

	// Check if valid handle
	if ( idx < 0 ) return NULL;
	if ( idx >= CANAL_PEAK_DRIVER_MAX_OPEN ) return NULL;
...
void CDllDrvObj::removeDriverObject( long h )
{
	...
	// Check if valid handle
	if ( idx < 0 ) return;
	if ( idx >= CANAL_PEAK_DRIVER_MAX_OPEN ) return;
