///////////////////////////////////////////////////////////////////////////////
// Name:        syslogtest.cpp
// Purpose:     LJP Print Daemon
// Author:      Ake Hedman
// Modified by:
// Created:     2005-02-25
// RCS-ID:      $Id: syslogtest.cpp,v 1.2 2005/06/01 19:58:44 akhe Exp $
// Copyright:   (c) 2004 Ake Hedman <akhe@eurosource.se>
// Version 0.0.1
// 
//
...
void appFrame::OnTimerRefresh( wxTimerEvent& event ) 
{
	...
	m_secondCounter++;
	int count;
	count = m_syslogobj.dataAvailable();

	wxstr.Printf( "Number of events = %d", count );
	...
	if ( count > 0 ) {	
		...
		canalMsg msg;
		if ( m_syslogobj.readMsg( &msg ) ) {
			
			for ( int i=0; i<msg.sizeData; i++ ) {
				wxstrdata += wxstrdata.Format(" %d", msg.data[ i ] );
			}

			msgstr.Printf("id=%d datacount=%d - %s\r\n",msg.id, msg.sizeData, wxstrdata );
			wxstr = msgstr + wxstr;
		}
