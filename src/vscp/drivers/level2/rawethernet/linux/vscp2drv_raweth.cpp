// vscp2drv_raweth.cpp : Defines the initialization routines for the DLL.
//
...
long CVSCPDrvApp::addDriverObject(CRawEthernet *psockcan)
{
	long h = 0;
	...
	for (int i = 0; i < VSCP_RAWETHERNET_DRIVER_MAX_OPEN; i++) {
		...
		if (NULL == m_prawethernetArray[ i ]) {
			...
			m_prawethernetArray[ i ] = psockcan;
			h = i + 1681;
			break;
...
CRawEthernet *CVSCPDrvApp::getDriverObject(long h)
{
	long idx = h - 1681;

	// Check if valid handle
	if (idx < 0) return NULL;
	if (idx >= VSCP_RAWETHERNET_DRIVER_MAX_OPEN) return NULL;
	return m_prawethernetArray[ idx ];
}
...
void CVSCPDrvApp::removeDriverObject(long h)
{
	...
	// Check if valid handle
	if (idx < 0) return;
	if (idx >= VSCP_RAWETHERNET_DRIVER_MAX_OPEN) return;

	LOCK_MUTEX(m_objMutex);
	if (NULL != m_prawethernetArray[ idx ]) delete m_prawethernetArray[ idx ];
	m_prawethernetArray[ idx ] = NULL;
	UNLOCK_MUTEX(m_objMutex);
}
