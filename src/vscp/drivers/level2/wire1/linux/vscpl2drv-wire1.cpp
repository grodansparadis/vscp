// vscpl2drv-wire1.cpp : Defines the initialization routines for the DLL.
//
...
static std::map<long, CWire1 *> g_ifMap;
...
_fini()
{
    ...
    for (std::map<long, CWire1 *>::iterator it = g_ifMap.begin();
         it != g_ifMap.end();
         ++it) {
        // std::cout << it->first << " => " << it->second << '\n';

        CWire1 *pif = it->second;
        if (NULL != pif) {
...
{
    std::map<long, CWire1 *>::iterator it;
    long h = 0;
...
VSCPClose(long handle)
{
    ...
    CWire1 *pdrvObj = getDriverObject(handle);
